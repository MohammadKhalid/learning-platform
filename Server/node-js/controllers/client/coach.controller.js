const { Sequelize, User, Company, DepartmentTag } = require('../../models');
...
const create = async function(req, res){
    ...
    // } 
        ...
        // console.log('DEPT IDSSSS', body.departments);
        // return ReE(res, 'Please select user companies.'); // temp
        ...
        // [err, assignedCompanies] = await to(newUser.addCompanies(companies));
        // if(err) return ReE(res, err, 422);

        // console.log('NEW USER COMPANIES', assignedCompanies);
        // add department assoc
        ...
        [err, companies] = await to(new Promise((resolve, reject) => {
            companies.forEach(async (company, index, array) => {
                ...
                // save departments
                // console.log('companyDepartments ID', company.id);
                // console.log('companyDepartments', companyDepartments[company.companyId]);

                console.log('COMP OBJJJJ', assignedCompany);
                ...
                if(assignedCompany && company.departments.length > 0) {
                    ...
                    console.log('DEPPPPPPPPPP ASSS', assignedCompanyDepartments);
                }
        ...
        // tag new user
        // [err, newUser] = await to(newUser.addAssignedCompanies(assignedCompanies, { through: { taggable: 'assignedCompany' } }));
        // if(err) return ReE(res, err, 422);
    }
