...
const answerQuestion = async function(req, res){
    ...
    if(!item || item.length < 1) return ReE(res, 'Not found!');
...
const update = async function(req, res){
    let item_id, err, item, user, data;
...
const submit = async function(req, res){
    ...
    if(data.submittedTo) {
        ...
        if(!err) {
            // add coach
            await item.addUser(coach, { through: { status: 'started' }});
